...
const RupeekFirstPage = ({searchParams}) => {
  ...
  const handleFirstPageSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}rupeekngold_new`, formData1);
  ...
  if (showSuccess) {
    ...
    return <RupeekSecondPage formState={formState} formData={formData} onBack={handleBack} />;
  }
