...
const IIFLPage = ({searchParams}) => {
  ...
  const validateForm = () => {
    const newErrors = {};
    const { fullName, mobileNumber, email, pinCode } = formState;
  ...
  const handleFirstPageSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}iiflgold_new`, formData1);
