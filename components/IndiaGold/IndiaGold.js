...
import IndiaGoldSecPage from "./IndiaGoldSecPage";
...
const IndiaGold = ({ searchParams }) => {
  ...
  const validateForm = () => {
    const newErrors = {};
    const { fullName, mobileNumber, panNumber, pinCode } = formState;
  ...
  const handleFirstPageSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}indiagold_new`,
        formData1
      );
...
//       newErrors.fullName = "Please enter your full name";
//     } else if (!/^[A-Za-z\s]{3,50}$/.test(fullName)) {
//       newErrors.fullName = "Name should only contain letters and spaces (3-50 characters)";
//     }
...
//       const response = await axios.post(`${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}indiagold_new`, formData1);
//       if(response.data.code === 0) {
//         console.log("The response is 0");
//         setStgOneHitId(response.data.obj.stgOneHitId);
//         setstgTwoHitId(response.data.obj.stgTwoHitId);
...
//   // Early return for success state
//   if (showSuccess) {
//     {console.log("The formState mobile number on first page is : ",formState.mobileNumber)}
//     return <IndiaGoldSecPage formState={formState} formData={formData} onBack={handleBack} />;
//   }
