...
const FirstPage = () => {
  ...
  const handleFirstNameChange = (e) => {
    ...
    if (/^[a-zA-Z]+$/.test(inputValue) || inputValue === "") {
      ...
      if (capitalizedFirstName !== lastName) {
        ...
        setErrorMessages({
          ...errorMessages,
          firstName: "First name and last name should not be the same",
        });
      }
    } else {
      setErrorMessages({
  ...
  const handleLastNameChange = (e) => {
    ...
    if (/^[a-zA-Z]+$/.test(inputValue) || inputValue === "") {
      ...
      if (firstName !== capitalizedLastName) {
        ...
        setErrorMessages({
          ...errorMessages,
          lastName: "First name and last name should not be the same",
        });
      }
    } else {
      setErrorMessages({
  ...
  const handleClickSendOTP = async () => {
    const errors = {
      ...
      userPhoneNumber:
        userPhoneNumber.length !== 10 ? "Mobile Number must be 10 digits" : "",
  ...
  const handleSubmit = async (firstdata) => {
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}chfronetendotpgenerator`,
        formData
      );
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}creditcardotp`,
        formData1
      );
      if (response.data.code === 0) {
        setStgOneHitId(response.data.obj.stgOneHitId);
  ...
  const handleVerification = async () => {
    ...
    try {
      ...
      const response1 = await axios.post(
        `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}verifyOTP`,
        formData
      );
  ...
  return (
    <>
      <div
        style={{
          background:
            "linear-gradient(rgb(0 0 0 / 7%), rgb(153 153 153 / 17%))",
        }}
      >
        <Animation />
        ...
        <div>
          <p
            ...
            className="agreepara scroll"
          >
            You hereby consent to CreditHaat being appointed as your authorized
            representative to receive your Credit Information from Experian for
            the purpose of accessing credit worthiness and availing pre-approved
            offers (“End Use Purpose”). You hereby agree to{" "}
            <a href="/experianterms">Terms and Conditions.</a> <br />I authorize
            CreditHaat and its representatives to Call, SMS or communicate via
            WhatsApp regarding my application. This consent overrides any
            registration for DNC / NDNC. I confirm I am in India, I am a major
            and a resident of India and I have read and I accept CreditHaat
            Privacy Policy Click here to read the{" "}
            ...
            >
              ...
              you provide your express consent to EarlySalary Services Private
              Limited(fibe) to access the credit bureaus and credit information
        ...
        >
          ...
          &nbsp;is a loan aggregator that helps users to find the best loan
          offers from a range of lender options. CreditHaat has partnered with
          45+ lending partners that can help users of all profiles avail loans
          from Rs.5000 to Rs.50Lac. CreditHaat accepts user’s profile
          information and runs its analytics engine to match them with the best
          fit lender for the user’s credit needs. In addition CreditHaat helps
          the user completes the loan application through guided support making
          the entire loan process seamless and convenient. CreditHaat allows
          users to avail Unsecured Personal Loans, Unsecured Business Loans,
          Gold Loans, Home Loans, LAP, unsecured and secured credit cards.
          CreditHaat thus functions as a single gateway for a user to meet their
          borrowing objectives. The team is based in Pune, Maharashtra and is
          one of the fastest growing digital aggregators of financial services.
        </p>
              ...
              {otpValues.map((value, index) => (
                <TextField
                  ...
                  style={{
                    ...
                    margin: "5px",
                    textAlign: "center",
                    border: otpVerificationFailed ? "2px solid red" : "",
                    borderRadius: otpVerificationFailed ? "7px" : "0px",
                  }}
                  inputProps={{
                    maxLength: 1,
                    style: { textAlign: "center" },
