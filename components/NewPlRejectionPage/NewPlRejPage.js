  ...
  const handleAdClick = async (productId, productLink) => {
    ...
    try {
      ...
      if (productLink.toLowerCase().includes("securedproducts")) {
        const storedMobile = mobileNumber || localStorage.getItem('mobileNumberForRejection');
        if (storedMobile) {
          // Append the mobile number and otpVerified flag to the URL.
          // Check if the URL already has query parameters
          if (productLink.indexOf('?') !== -1) {
            productLink += `&otpVerified=true&mobileNumber=${encodeURIComponent(storedMobile)}`;
          } else {
            productLink += `?otpVerified=true&mobileNumber=${encodeURIComponent(storedMobile)}`;
          }
        }
