...
import NewBlListPage from "../NewPl_Apply_Prime_Journey/NewPlPrimeListPage";
...
import NewPlPage2 from './PlApplyPrimeSecond';
...
const PlApplyPrimeFirst = ({ params, searchParams }) => {
...
const handleInputChange = (e) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      formData1.append("dsa", formData.dsa);
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}chfronetendotpgenerator_NewPlApplyPrime`,
        formData1
      );
...
const verify_otp_credithaat_from_backend = async (e) => {
  ...
  try {
    ...
    console.log("The otp before verifying it is  :",upotp);
    // alert(upotp);
    ...
    if (response.data.code === 0 || response.data.code === 1 || response.data.code === 2 || response.data.code === 3) {
      ...
      if (formData.profession === "Salaried") {
        ...
        setActiveContainer("NewPlPage2"); // Display NewPlApplyDS if self-employed
      }
    } else {
      setOtpLoader(false);
...
const handleProfessionChange = (selectedOption) => {
  ...
  applicationPopup && <ApplicationPopup link={link}/>
...

      {
        ...
        <NewPlPage2 cpi={cpi} lenderProduct={lenderProduct} firstName={firstName} lastName={lastname} mainFormData={formData} dobFlag={dobFlag} residentialPincodeFlag={residentialPincodeFlag} genderFlag={genderFlag} addressFlag={addressFlag} setActiveContainer={setActiveContainer} getLendersList={getLendersList} />
      }
...
export default PlApplyPrimeFirst;
