...
const OTPComponent = ({ mobile }) => {
  ...
  const verifyOTP = async (e) => {
    ...
    try {
      ...
      formData1.append("deviceId", getDeviceId());

      console.log("Before the verifyOtpNewPersonal");

      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}verifyOTPEmbedded`,
        formData1
      );
  ...
  const OTPGenerate = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_REACT_APP_BASE_URL}chEmbeddedList_OTPGenerate`,
        formData1
      );
